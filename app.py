...
def fetch_raw_logs(list_item_id):
    try:
        ...
        return raw_resp.json().get("listItemStatusLogs", [])
    except requests.exceptions.RequestException as e:
        st.error(f"Request failed: {e}")
        return []
    ...
    if "selected_row" in st.session_state:
        ...
        if list_item_id:
            with st.spinner("Fetching..."):
                items = fetch_raw_logs(list_item_id)
            ...
            # deleted entries
            deleted = [i for i in items if isinstance(i, dict) and i.get("status") == "deleted"]
            if deleted:
                st.markdown("**Deleted**")
                for d in deleted:
